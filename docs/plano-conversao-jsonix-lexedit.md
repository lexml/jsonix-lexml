...
- ✅ **Resolvido**: `mappings:jsonix-compiler` ([package.json](../package.json)) passou a usar `mvn install:install-file ... && mvn exec:java` em vez de `java -jar` direto — reaproveita as dependências já declaradas em `pom.xml` (`javax.activation:activation:1.1.1`, `jaxb-api/core/impl:2.3.0`), resolvendo o `ClassNotFoundException: javax.activation.DataSource` sem precisar de JDK 8, Docker ou vendorizar jar nenhum. Único pré-requisito novo: `mvn` precisa estar no `PATH` (decisão explícita: sem Maven Wrapper).
- ✅ Um segundo problema, só visível em JDK ≥ 16 (descoberto ao testar de verdade, não estava previsto): o JAXB RI de 2014 embutido no compilador usa reflexão para definir classes em runtime, bloqueada por padrão desde o JEP 396 (`InaccessibleObjectException: module java.base does not "opens java.lang"`). Resolvido com **[.mvn/jvm.config](../.mvn/jvm.config)** (`--add-opens java.base/java.lang=ALL-UNNAMED`), lido automaticamente pelo launcher do Maven — não exige nenhuma mudança em como `mvn` é invocado, e é inofensivo em JDK 9-15 (a flag existe desde o JDK 9).
- ✅ Testado de ponta a ponta neste ambiente: `npm run build:mappings` com JDK 17 (sem Docker, sem JDK 8) gera os mappings corretamente e `npm test` passa. Única ressalva: a máquina de teste tem o `~/.m2/settings.xml` apontando para o mirror interno do Senado (`www20.senado.gov.br`), inalcançável do sandbox usado aqui — a validação final foi feita apontando temporariamente para a Maven Central pública. Numa máquina real do Senado, dentro da rede/VPN, o mirror interno deve resolver normalmente.

### Fase 1 — Gerar e revisar o mapping do `lexedit.xsd` ✅ implementado
- ✅ Mappings regenerados em `mappings/` (gitignorado, como já era o padrão do projeto), por fim via `npm run build:mappings` (Maven + JDK 17, ver Fase 0): `br_gov_lexml_lexedit__1.js`/`.jsonschema` novos, `br_gov_lexml__1.js` atualizado (propriedade `any` de `MetadadoProprietario` sem `allowTypedObject: false`, confirmado por inspeção do arquivo gerado).
...
- `npm run build:mappings` agora depende de `mvn` estar no `PATH` (decisão explícita do time: sem Maven Wrapper) e de acesso de rede a um repositório Maven (Central ou o mirror interno do Senado). Antes dependia de `java` estar no PATH; a troca é razoável dado que o projeto já depende de Maven para o build "oficial" (Docker), mas vale deixar registrado como pré-requisito.
...
## Critérios de aceite
- ✅ `npm run build:mappings` gera `br_gov_lexml_lexedit__1.js` e atualiza `br_gov_lexml__1.js`, reprodutível com JDK 11+ sem Docker (testado com JDK 17 neste ambiente, via Maven — ver Fase 0).
