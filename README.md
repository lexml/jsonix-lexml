...
O ponto de extensão `MetadadoProprietario` do LexML (`xsd:any`) é usado pelo LexEdit para anexar metadados próprios (autoria, comentários, revisões etc. — ver [schemas/lexedit.xsd](schemas/lexedit.xsd)). Esse conteúdo é convertido para objetos JSON tipados, não para XML serializado: o elemento `<lexedit:Metadado>` aparece dentro de `metadadoProprietario[].any[].value`, com `TYPE_NAME: "br_gov_lexml_lexedit__1.Metadado"`.

```js
    const json = jsonixLexml.toJSON(xml);
    const metadadoProprietario = json.value.metadado.metadadoProprietario[0];
    const lexedit = metadadoProprietario.any[0].value;

    console.log(lexedit.aplicacao);                              // "LexEdit"
    console.log(lexedit.autoria.parlamentares.parlamentar[0].nome); // "Davi Alcolumbre"
```

Veja [samples/documento-articulado-exemplo.xml](samples/documento-articulado-exemplo.xml) para um exemplo completo, e [docs/plano-conversao-jsonix-lexedit.md](docs/plano-conversao-jsonix-lexedit.md) para detalhes da integração (em especial a observação sobre `RevisaoArticulacao`, cujo conteúdo é uma união entre 17 tipos de dispositivo — apenas um fica preenchido por vez).
